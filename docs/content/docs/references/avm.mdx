...
newest known installable Solana CLI version that satisfies the requirement,
using Anza-hosted installers first and legacy Solana GitHub release assets for
older versions that are no longer hosted by Anza. Use an exact comparator such
as `=3.1.10` to require one specific known version.
...
requirement, AVM considers known installable Solana CLI versions satisfying
that requirement and prefers one whose platform-tools rustc can compile the
locked program dependency graph.
