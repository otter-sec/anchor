...
SUBCOMMANDS:
    ...
    nightly        Enable or disable the Anchor nightly channel
```
...
Enable the Anchor nightly channel. AVM reads the latest nightly date and uses
the successful Anchor build from the preceding calendar day. While nightly mode
is enabled, invoking `anchor` checks for a newer eligible build at most once per
hour. These checks only replace the nightly Anchor binary; the installed AVM is
left unchanged.

```shell
# Enable Anchor nightly
avm nightly

# Return to normal Anchor version resolution
avm nightly --disable
```
...
```shell
avm self-update [--pre-release] [--bleeding-edge] [--enable-auto | --disable-auto]
```
...
| `--enable-auto` | Enable automatic stable AVM updates and update immediately |
| `--disable-auto` | Disable automatic AVM updates without updating |
...
# Automatically install future stable AVM updates
avm self-update --enable-auto

# Return to warning-only update checks
avm self-update --disable-auto
```

By default, `avm` passively warns when it detects a newer stable version. With
automatic updates enabled, the same periodic check installs that release
instead.
