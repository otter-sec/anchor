...
The installer sets up AVM and installs the latest stable Anchor CLI. It does not
enable the Anchor nightly channel.

Check that AVM and Anchor CLI were installed successfully:
...
To opt in to Anchor nightly builds, run:
...
nightly and leaves the installed AVM unchanged. Disable it with
`avm nightly --disable`.

You can install and select a specific Anchor CLI version by specifying the
version number:
...
AVM warns when a newer stable version of itself is available. Run
`avm self-update --enable-auto` to install future stable AVM updates
automatically, or `avm self-update --disable-auto` to return to warning-only
checks.
